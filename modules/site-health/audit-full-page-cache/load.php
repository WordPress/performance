...
function perflab_afpc_page_cache_no_cache_plugin_test( $response, $error_message, $error_code ) {
	$response['badge']['color'] = 'red';
	$response['label'] = esc_html__( 'Unable to detect the presence of page caching', 'performance-lab' );
	...
	$response['description'] = wp_kses_post( "<p>$error_info</p>" ) . $response['description'];
	return $response;
}
...
function perflab_afpc_page_cache_test() {
	...
	if ( is_wp_error( $page_cache_detail ) ) {
		return perflab_afpc_page_cache_no_cache_plugin_test( $result, $page_cache_detail->get_error_message(), $page_cache_detail->get_error_code() );
	}

	if ( 'recommended' === $page_cache_detail['status'] ) {
		$badge_color = 'orange';
		$status      = $page_cache_detail['status'];
		$label       = __( 'Page caching is not detected but the server response time is OK', 'performance-lab' );
	} elseif ( 'good' === $page_cache_detail['status'] ) {
		$badge_color = 'green';
		$status      = $page_cache_detail['status'];
		$label       = __( 'Page caching is detected and the server response time is good', 'performance-lab' );
	} else {
		$badge_color = 'red';
		$status      = $page_cache_detail['status'];
		if ( empty( $page_cache_detail['headers'] ) && ! $page_cache_detail['advanced_cache_present'] ) {
			$label = __( 'Page caching is not detected and the server response time is slow', 'performance-lab' );
		} else {
			$label = __( 'Page caching is detected but the server response time is still slow', 'performance-lab' );
		}
	}
	...
	if ( ! is_wp_error( $page_cache_detail ) ) {
		...
		if ( empty( $page_cache_detail['response_time'] ) ) {
			...
			$threshold = get_good_response_time_threshold();
