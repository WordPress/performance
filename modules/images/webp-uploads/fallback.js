...
( function ( document ) {
		...
		for ( let i = 0; i < media.length; i++ ) {
			try {
				...
				for ( let j = 0; j < images.length; j++ ) {
					...
					let srcset = images[ j ].getAttribute( 'srcset' );

					for ( let k = 0; k < sizes_keys.length; k++ ) {
						const media_sizes_sources =
	...
	const loadMediaDetails = function ( nodes ) {
		...
		for ( let i = 0; i < nodes.length; i++ ) {
			...
			const attachment = node.className.match( /wp-image-(\d+)/i );
			if (
				attachment &&
				attachment[ 1 ] &&
		...
		for (
			...
			const pageIds = [];
			for ( let j = 0; j < 100 && j + page * 100 < ids.length; j++ ) {
