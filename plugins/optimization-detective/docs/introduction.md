...
When an `od_url_metrics` post has not been modified for three (3) months, it will then be garbage-collected since it is likely the original URL for the metrics has gone away. If you have a site that gets very little traffic and you want to increase the TTL for garbage collection, you can use the `od_url_metric_garbage_collection_ttl` filter to increase this.
