...
When an `od_url_metrics` post has not been updated within the default value i.e., a month or the filtered value via `od_garbage_collection_ttl_period` then it is garbage-collected, as it is likely that the original URL has gone away
